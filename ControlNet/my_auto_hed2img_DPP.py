...
def setup(rank, world_size):
    ...
    torch.cuda.set_device(rank)  # sets device for each process
...
def load_ddp_model(rank):
    ...
    model.load_state_dict(load_state_dict('./checkpoints/finetuned-instr-oilpaint-hed-controlnet-b16-s256-2024-12-12_16-17-04-epoch=19.ckpt', location=f'cuda:{rank}'))
    model = model.to(rank)
...
def process_image(rank, model, ddim_sampler, input_image_path, prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, low_threshold, high_threshold):
    with torch.no_grad():
        ...
        apply_hed.netNetwork = apply_hed.netNetwork.to(rank)
...
def process_directory(rank, world_size, input_dir, output_dir, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta, low_threshold, high_threshold):
    ...
    start_index = 0 # CHECK # CHECK
    # image_files = image_files[start_index:]  # start from the desired index, dividing by 2600 intervals
    ...
    for image_path, image_file in tqdm(data_loader, desc=f"Processing images on GPU {rank}"):
        ...
        if num_augs==0: # don't augmented the overrepresented classes
            continue
        ...
        # each augmentation one by one with a different seed
        for aug_idx in range(num_augs):
            ...
            hed_maps_dir = os.path.join(os.path.dirname(output_dir), "hed_maps")
            os.makedirs(hed_maps_dir, exist_ok=True)
            for idx, result in enumerate(results):
                if idx==0:
                    pass
                    # result = cv2.cvtColor(result, cv2.COLOR_BGR2RGB) 
                    # output_file = os.path.join(hed_maps_dir, f"{image_file[0].rsplit('.', 1)[0]}_map_{aug_idx + 1}.png")
...
def main():
    ...
    output_dir = '/home/woody/iwi5/iwi5215h/masterarbeit/repos/odor-images/FINAL_FINAL_flip_controlnet_hed_bbox_images_afterfinetuning20_b16_sdm256_classimbalance_0901'
