...
from class_imbalance import ImageAugmentationPrioritizer
import shutil
...
prioritizer = ImageAugmentationPrioritizer(
    '/home/woody/iwi5/iwi5215h/masterarbeit/repos/odor-images/my_modified_train_split.json'
)
...
def run():
    ...
    parser.add_argument("--mask_dir", type=str, default="/home/woody/iwi5/iwi5215h/masterarbeit/repos/odor-images/reversed_saliency_masks/", help="Directory containing masks")
    ...
    start_idx = 0
    for image_file in tqdm(image_files[start_idx:],desc="Performing inpainting on ODOR images"):
        ...
        # Get number of augmentations for this image
        num_augmentations = augmentation_counts.get(image_file, 0)  

        if num_augmentations == 0:
            # If no augmentations are needed, copy the original image to the output directory
        ...
        for mask_file in corresponding_masks:
            ...
            class_name = os.path.splitext(mask_file)[0].split('_')[-3]
            ...
            for idx in range(num_augmentations):
                ...
                if args.num_samples==1:
                    for img in result:
                        new_image_name = f"{base_name}_{class_name}_{ann_id}_synthesized_{idx + 1}.png"
                        img.save(os.path.join(args.output_dir, new_image_name))
                else:
                    for index, img in enumerate(result):
                        new_image_name = f"{base_name}_{class_name}_{ann_id}_synthesized-{index}_{idx + 1}.png"
                        img.save(os.path.join(args.output_dir, new_image_name))
