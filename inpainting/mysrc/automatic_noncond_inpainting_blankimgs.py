...
def run():
    ...
    for image_file in tqdm(image_files,desc="Performing inpainting on ODOR images"):
        ...
        if image_file.startswith("blank"):

            corresponding_masks = [m for m in mask_files if m.startswith(image_file.rsplit('.', 1)[0])]

            # nonobj_classes = ["Knights", "Children", "Squats", "Bards" ,"Coat of arms", "Crown", "Tiara", "Armor", "Harp", "Violin", "Lute", "Throne", "Fountain", "Quill", "Tapestry", "Shield", "Helmet", "Banner", "Cloak", "Lyre", "Hourglass", "Column", "Pedestal", "Arch", "Spyglass", "Carriage", "Gauntlet", "Cameo", "Relief", "Statue", "Tunic", "Fresco", "Bas-relief"]
            ...
            for mask_file in corresponding_masks:
                ...
                if args.num_samples==1:
                    ...
                    for index, img in enumerate(result):
                        new_image_name = f"{base_name}-{index}.png"
                        img.save(os.path.join(args.output_dir, new_image_name))
        else:
            # Save the image as it is in the output directory
